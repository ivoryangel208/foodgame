    ...
    <script>
        ...
        function updateUI() {
            ...
            plants.forEach((plant, index) => {
                ...
                plantItem.textContent = `${plant.name} (${plant.thc} THC) - Status: ${plant.status}`;
                inventoryList.appendChild(plantItem);
            });
        ...
        document.getElementById("harvest-button").addEventListener("click", () => {
            if (plants.length > 0 && !isPlantReady) {
        ...
        document.getElementById("make-joint-button").addEventListener("click", () => {
            ...
            if (plant.status === "Ready to Harvest") {
                ...
                plant.status = "Processed - Joint Made";
                updateUI();
            } else {
                alert("You need to harvest the plant first!");
            }
        });
        ...
        document.getElementById("make-edible-button").addEventListener("click", () => {
            ...
            if (plant.status === "Ready to Harvest") {
                ...
                plant.status = "Processed - Edible Made";
                updateUI();
            } else {
                alert("You need to harvest the plant first!");
            }
        ...
        document.getElementById("make-dab-button").addEventListener("click", () => {
            ...
            if (plant.status === "Ready to Harvest") {
                ...
                plant.status = "Processed - Dab Made";
                updateUI();
            } else {
                alert("You need to harvest the plant first!");
            }
        ...
        document.getElementById("sell-button").addEventListener("click", () => {
            const plant = plants[plants.length - 1];
            if (plant.status === "Processed - Joint Made" || plant.status === "Processed - Edible Made" || plant.status === "Processed - Dab Made") {
                money += plant.price;
                plants.pop();
                alert("Weed Sold!");
                updateUI();
            } else {
                alert("You need to process the weed before selling it!");
            }
        });

        // Travel
        document.getElementById("travel-button").addEventListener("click", () => {
            const locations = ["Farm", "Store", "Processing Plant"];
            const currentLocation = locations[Math.floor(Math.random() * locations.length)];
            alert(`Traveling to ${currentLocation}`);
            updateUI();
        });
